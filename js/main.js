...
(function () {
  ...
  /* Tema (kayıt yoksa sistem tercihine bak) */
  var themeBtn = document.getElementById("themeBtn");
  function temayiUygula(acik) {
    if (acik) {
  ...
  try {
    ...
    if (kayitli === "acik") temayiUygula(true);
    else if (!kayitli && window.matchMedia("(prefers-color-scheme: light)").matches) temayiUygula(true);
  } catch (e) {}
  ...
  /* Profil 3D eğilmesi (sadece ince işaretçi) */
  try {
    var ppKutu = document.querySelector(".pp-kutu");
    if (ppKutu && window.matchMedia("(pointer:fine)").matches && !dusukHareket) {
      ppKutu.addEventListener("mousemove", function (e) {
        var r = ppKutu.getBoundingClientRect();
        var dx = (e.clientX - r.left) / r.width - 0.5;
        var dy = (e.clientY - r.top) / r.height - 0.5;
        ppKutu.style.transform = "perspective(700px) rotateY(" + (dx * 14).toFixed(1) + "deg) rotateX(" + (-dy * 14).toFixed(1) + "deg)";
      });
      ppKutu.addEventListener("mouseleave", function () { ppKutu.style.transform = ""; });
    }
  } catch (e) {}
    ...
    fetch("https://api.github.com/users/Parsozkn").then(function (r) {
      ...
      try {
        ...
        arr.forEach(function (r) {
          var d = typeof r.language === "string" && r.language ? r.language : null;
          if (d) sayac[d] = (sayac[d] || 0) + 1;
        });
        var diller = Object.keys(sayac).sort(function (a, b) { return sayac[b] - sayac[a]; }).slice(0, 5);
        var dListe = document.getElementById("ghDiller");
        dListe.textContent = "";
        if (!diller.length) {
          var b = document.createElement("li");
          b.textContent = "Henüz dil verisi yok.";
          dListe.appendChild(b);
        }
        diller.forEach(function (d) {
          var oran = Math.round(sayac[d] / arr.length * 100);
          var li = document.createElement("li");
          var ad = document.createElement("span");
          ad.textContent = d;
          var cubuk = document.createElement("span");
          cubuk.className = "cubuk";
          var dolgu = document.createElement("i");
          cubuk.appendChild(dolgu);
          var yuzde = document.createElement("span");
          yuzde.className = "yuzde";
          yuzde.textContent = "%" + oran;
          li.appendChild(ad); li.appendChild(cubuk); li.appendChild(yuzde);
          dListe.appendChild(li);
          requestAnimationFrame(function () { dolgu.style.width = oran + "%"; });
        });
      } catch (e) {}
